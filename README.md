...
Проект представляет собой ETL-пайплайн для обработки данных о продажах игр из трёх источников (CSV, JSON, Excel) с последующей аналитикой и визуализацией.
# 📁 Структура проекта
```text
...
Основной скрипт — main.py. Он поддерживает три режима работы.
Режимы работы
Режим	Описание	Пример
auto 
(по умолчанию)	Проверяет наличие файлов в data/raw. Если есть — использует их. Если нет — генерирует новые.	
```bash python main.py ```
gen	
Всегда генерирует новые данные (файлы с префиксом g_) и запускает пайплайн.	
```bash python main.py --mode gen```

local	
Требует наличия пользовательских файлов в data/raw. Если их нет — завершается с ошибкой.	
```bash python main.py --mode local ```
